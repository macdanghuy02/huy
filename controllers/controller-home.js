...
module.exports.samsung = function (req, res) {
    Product.find({ cateProduct: 'samsung' }, function (err, data) {
        if (err) {
            res.json({ "kq": 0, "errMsg": err });
        } else {
            Session.findOne({ _id: req.session.views }, function (err, data1) {
                if (err) {
                    ...
                    res.render('./paperuser/samsung', { listproduct: data, listdata: data1, errs: req.flash('message','mua hang thanh cong') })
                }
            });
        }
    })
...
module.exports.apple = function (req, res) {
    Product.find({ cateProduct: 'apple' }, function (err, data) {
        if (err) {
            res.json({ "kq": 0, "errMsg": err });
        } else {
            Session.findOne({ _id: req.session.views }, function (err, data1){
                if (err) {
                    ...
                    res.render('./paperuser/apple', { listproduct: data, listdata:data1, errs: req.flash('message') });
                }
            })
        }
    })
