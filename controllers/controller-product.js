...
module.exports.addProduct = function (req, res) {
    Category.find(function (err, data) {
        if (err) {
            ...
            res.render("product/add", { listcategory: data, id: req.cookies.loginid,errs1: req.flash('message') });
        }
    })
};
    ...
    upload(req, res, function (err) {
        ...
        if (!req.file) {
            ...
            if (err instanceof multer.MulterError) {
                ...
                product.save(function (err) {
                    if (err) {
                        req.flash('message', 'Thêm thất bại');
                        res.redirect('/admin/product')
                        return;
                    } else {
                        req.flash('message', 'Thêm thành Công');
        ...
        if (err) {
            ...
            Product.findById(req.params.id, function (err, data) {
                if (err) {
                    ...
                    res.render('product/edit-pro', { Product: data, listcategory: pro,id: req.cookies.loginid, errs1: req.flash('message') })
                }
            })
